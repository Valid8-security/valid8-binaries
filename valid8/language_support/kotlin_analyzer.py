"""Kotlin language security analyzer."""
...
PATTERN_RULES: Dict[str, PatternRule] = {
    ...
    "detect_sql_injection": PatternRule(
        ...
        patterns=[
            r"\.execute\s*\(",
            r"rawQuery\s*\(",
            r"compileStatement\s*\(",
        ],
        evidence=["request", "params", "+", "format"],
        confidence=0.85,
    ),
    ...
    "detect_information_disclosure": PatternRule(
        ...
        patterns=[
            r"Log\.d.*(password|secret)",
            r"println.*(password|secret)",
        ],
        confidence=0.7,
        advanced=True,
    ),
...
class KotlinAnalyzer(LanguageAnalyzer, UniversalDetectors):
    """Security analyzer for Kotlin source files."""
    ...
    def analyze(self, code: str, filepath: str) -> List[Vulnerability]:
        self.logger.debug("Starting Kotlin analysis", extra={"filepath": filepath})
