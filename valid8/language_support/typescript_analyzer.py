...
"""TypeScript language security analyzer."""
...
PATTERN_RULES: Dict[str, PatternRule] = {
    "detect_command_injection": PatternRule(
        ...
        evidence=["req", "query", "body", "params", "input"],
        confidence=0.9,
    ),
    ...
    "detect_sql_injection": PatternRule(
        ...
        evidence=["req", "query", "body", "params"],
        confidence=0.85,
    ),
    ...
    "detect_no_sql_injection": PatternRule(
        name="detect_no_sql_injection",
        cwe="CWE-943",
        severity="high",
        title="NoSQL Injection",
        description="NoSQL query built directly from user input.",
        patterns=[
            r"db\.collection.*find\s*\(",
            r"MongoClient",
        ],
        evidence=["req", "query", "body"],
        confidence=0.75,
    ...
    "detect_information_disclosure": PatternRule(
        ...
        patterns=[
            r"console\.log.*(password|secret|token)",
            r"res\.send.*(password|secret|token)",
        ],
        confidence=0.7,
        advanced=True,
    ),
...
class TypeScriptAnalyzer(LanguageAnalyzer, UniversalDetectors):
    """Security analyzer for TypeScript source files."""
    ...
    def get_supported_cwes(self) -> List[str]:
        return [
            ...
            "CWE-601", "CWE-611", "CWE-798", "CWE-918", "CWE-943",
        ]

    def analyze(self, code: str, filepath: str) -> List[Vulnerability]:
        self.logger.debug("Starting TypeScript analysis", extra={"filepath": filepath})
    ...
    def detect_no_sql_injection(self, code: str, filepath: str) -> List[Vulnerability]:
        return self._run_rule("detect_no_sql_injection", code, filepath)
